...
def original_save_dicom_image_as_png(dicom_filename, png_output, bitdepth=12):
    """
    ...
    with open(png_output, 'wb') as f:
        ...
        writer.write(f, np.array(image, dtype=int).tolist())



def dicom_png_read(file_name):
    file_name_dcm = file_name[0:-3] + 'dcm'
    print(file_name_dcm)
    original_save_dicom_image_as_png(file_name_dcm, file_name)
